...
namespace Yc
{
    ...
    // 其中 U 默认是value_with_cookie<typename std::allocator_traits<Allocator>::value_type, Cookie>
    // 也就是外层容器的value_with_cookie<T, Cookie>
    ...
    class allocator_with_cookie : public allocator_with_cookie_base<Allocator>
    {
        ...
        using infact_allocator = std::allocator_traits<Allocator>::template rebind_alloc<U>;
    public:
        template<class T>
        ...
        using allocator_with_cookie_base<Allocator>::allocator_with_cookie_base;
        ...
        allocator_with_cookie select_on_container_copy_construction()noexcept
        {
        ...
        {
            infact_allocator alloc = this->get_allocator();
        ...
        void construct(T* xp, Args&&... args)
        {
            ...
            {
                typename std::allocator_traits<Allocator>::template rebind_alloc<T> alloc{ this->get_allocator() };
                std::allocator_traits<Allocator>::template rebind_traits<T>::construct(alloc, xp, std::forward<Args>(args)...);
            }
        }

        template<class T, class Arg>
        void construct(T* xp, std::pair<cookie_wrapper<Cookie>, Arg> arg)
        {
            if constexpr (is_cookie_allocator && std::is_same_v<T, U>)
            {
                auto&& alloc = this->get_allocator();
                value_with_cookie<typename std::allocator_traits<Allocator>::value_type, Cookie>* p = xp;
                std::allocator_traits<Allocator>::construct(alloc, std::addressof(p->value()), std::forward<Arg>(arg.second));
                p->cookie() = arg.first.cookie;
            }
        }
        ...
        void destroy(T* xp)
        {
            ...
            {
                typename std::allocator_traits<Allocator>::template rebind_alloc<T> alloc{ this->get_allocator() };
                std::allocator_traits<Allocator>::template rebind_traits<T>::destroy(alloc, xp);
            }
        }
