# `Yc::parent_aware_binary_tree<T,Allocator>::insert`

```C++
parent_aware_binary_tree insert(edge_const_proxy p, const T& value); //(1)
```

```C++
parent_aware_binary_tree insert(edge_const_proxy p, T&& value); //(2)
```
...
原先该 _树位置_ 上的 _二叉树_ 。如果原先该 _树位置_ 无 _二叉树_ ，那么返回一个空的 `parent_aware_binary_tree` 对象。
...
若抛出了异常（例如由构造函数），则保留 `parent_aware_binary_tree` 不修改，如同未曾调用过此函数（[强异常保证](https://zh.cppreference.com/w/cpp/language/exceptions)）。
