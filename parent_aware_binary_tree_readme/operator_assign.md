# `Yc::parent_aware_binary_tree<T,Allocator>::operator=`

```C++
parent_aware_binary_tree& operator=(const parent_aware_binary_tree& b); //(1)
```

```C++
parent_aware_binary_tree& operator=(parent_aware_binary_tree&& other)noexcept(/* 见下文 */); //(2)
```

以 `other` 的内容替换 `*this` 的内容。

- (1) 拷贝赋值运算符。递归复制 `b` 保有的所有 _二叉树_ 到 `*this` 。提供强异常安全保障：若复制过程中抛出异常，`*this` 会回退到赋值前的状态。
- (2) 移动赋值运算符。通过交换子树转移 `other` 保有的 _二叉树_ 的所有权。交换时会维护父节点指针的正确性。若分配器不支持传播且不相等，则通过递归移动构造新树后交换。
...
||||
|-:|-|:-|
|`b`|-|要赋值其所含 _二叉树_ 的 `parent_aware_binary_tree` 对象|
|`other`|-|要移动其所含 _二叉树_ 的 `parent_aware_binary_tree` 对象|
...
- (2) 常数（分配器传播或相等时）或与 `other` 中的节点数成正比（分配器不传播且不相等时）。
